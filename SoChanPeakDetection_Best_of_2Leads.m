...
[onset_pks1,onset_locs1,R_pks1,R_locs1] = Func_SoChanPeakDetection_R_Point_Detection_with_Overlap_No_Windows_Adaptive_Trigger(c1);
...
[onset_pks2,onset_locs2,R_pks2,R_locs2] = Func_SoChanPeakDetection_R_Point_Detection_with_Overlap_No_Windows_Adaptive_Trigger(c2);

% Initialization factors
i = 1;
k = 1;

% Final R locs and its confidence factor
R_locs = [];
R1_high = [];
R1_low = [];
R2_low = [];
...
low_conf_locs = find(conf_fac == 50);                      % Finding all locs with 50% confidence
R_conf_fac_25 = [];
R_conf_fac_75 = [];
...
for i = 1:length(low_conf_locs)                           % For all those low confidence locs
  ...
  if RRI(low_conf_locs(i)) < RRI_low || RRI(low_conf_locs(i)) > RRI_high                % If current RRI doesnt fall within acceptable threshold
    conf_fac(low_conf_locs(i)) = 25;                      % set confidence factor to 25%, else
    R_conf_fac_25 = [R_conf_fac_25,R_locs(low_conf_locs(i))];    
  else 
    conf_fac(low_conf_locs(i)) = 75;                      % set confidence factor to 75%
    R_conf_fac_75 = [R_conf_fac_75,R_locs(low_conf_locs(i))];
  endif
endfor
...
R_conf_fac_25_markers = (mean(c2)-10)*ones(1,length(R_conf_fac_25));
R_conf_fac_75_markers = (mean(c2)+10)*ones(1,length(R_conf_fac_75));
...
plot(c2);
hold on
plot(R_conf_fac_25,R_conf_fac_25_markers,'marker','v', 'color', 'y', 'MarkerSize', 6);
hold on
plot(R_conf_fac_75,R_conf_fac_75_markers,'marker','v', 'color', 'm', 'MarkerSize', 6);
hold on
