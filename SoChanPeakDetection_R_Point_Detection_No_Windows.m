...
% Edits 
% Implementation of So and Chan (Max Slope detection method) with no windows
...
%slope = zeros(1,c_length);
%  for i = 4:c_length-3
%      slope(i) = (-3*c(i-3) - 2*c(i-2) - c(i-1) + c(i+1) + 2*c(i+2)) + 3*c(i+3);    % Slope calculation of the signal      
%  endfor
