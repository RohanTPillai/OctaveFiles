...
% Edits 
% Correlation between (R peak - onset peak) value and the wrong peaks is calculated on the No-Windows-Adaptive-Trigger algorithm 
% Implemented LPF of 10Hz
...
while(i < c_length)
  ...
  if (slope(i) > slope_threshold && slope(i+1) > slope_threshold)
    ...
    % Updating maxi 
    R_Onset_Diff = [R_Onset_Diff,R_pk - onset_pk];    
...
title('R-Point Detection based QRS Detection with No Window');
...
%plot(R_locs,R_pks,'marker','v', 'color', 'g', 'MarkerSize', 6);
%hold on
%plot(R_locs,R_Onset_Diff,'marker','v', 'color', 'm', 'MarkerSize', 3);
%text(R_locs,R_Onset_Diff,sprintf("%6.3f",R_Onset_Diff));
